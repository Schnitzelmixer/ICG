...
export default function phong(color: Vector, intersection: Intersection, lightPositions: Array<Vector>, cameraPosition: Vector, phongValues: PhongValues): Vector {
	...
	for (let i = 0; i < lightPositions.length; i++) {
		//lightDirection geht von intersection zur Lichtquelle
		let lightDirection = lightPositions[i].sub(intersection.point);
		//reflectDirection geht von intersection in die reflektierte Richtung
		...
		//Berechnet specular nur, wenn es nicht auf der Rückseite des Objektes ist,
		//also das .dot zwischen intersection.normal und lightDirection > 0 ist. (dot-Product ist negativ, wenn zwischen Vektoren über 90° sind)
