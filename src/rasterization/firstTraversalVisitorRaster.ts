	...
	visitCameraNode(node: CameraNode, active: boolean) {
		if (active) {
			...
			let cameraRasteriser = {
				//mulVec(Übergabeparameter sind quasi lokale Objektkoordinaten) -> werden durch mulVec mit toWorld in World Space transformiert
				eye: toWorld.mulVec(new Vector(0, 0, 0, 1)), // origin
				center: toWorld.mulVec(new Vector(0, 0, -1, 1)),
				//up wird benötigt, um die Ausrichtung festzustellen -> nicht möglich, wenn nur eye und center gegeben sind.
